...
def gestion_users():
    ...
    # 🔹 Récupération des utilisateurs avec leur rôle
    users = db.execute('''
        SELECT 
            u.id,
            u.nom, 
    ...
    # 🔹 Récupération de tous les rôles (pour le select du modal)
    roles = db.execute("SELECT user_role FROM role").fetchall()

    # 🔹 Récupération de tous les groupes (pour le select du modal)
    groupes = db.execute("SELECT id, nom FROM groupe").fetchall()

    # 🔹 Conversion des résultats en liste de dicts
    users_list = []
    for row in users:
        user = dict(row)
    ...
    # 🔹 Conversion des rôles et groupes en listes simples pour le template
...
def add_user():
    ...
    password = request.form.get('password')
    groupe_id = request.form.get('groupe')  # ID du groupe (peut être None)
    ...
    # Si le rôle n'est pas étudiant, ignorer le groupe
    if role_name != 'etudiant':
        groupe_id = None
    else:
        # Vérifier que le groupe existe dans la base
        groupe_row = db.execute(
            "SELECT id FROM groupe WHERE id = ?",
            (groupe_id,)
        ).fetchone()
...
def edit_user():
    ...
    # 🔹 Gestion du groupe selon le rôle
    if role_name != 'etudiant':
        groupe_id = None
    else:
    ...
    # 🔹 Mise à jour de l'utilisateur selon que le mot de passe est rempli ou non
    if password:
        hashed_password = generate_password_hash(password)
        db.execute(
            """
...
def gestion_examens():
    ...
    # ================= Matières avec nom complet de l'enseignant =================
    c.execute("""
        SELECT 
            m.id, 
            m.nom, 
